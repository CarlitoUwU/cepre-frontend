    ...
    <div className="flex flex-col md:flex-row h-[85vh] m-5 mt-25">
      ...
      <div className="w-full md:w-1/6 bg-gray-200 p-6 flex md:flex-col flex-wrap gap-4 md:gap-6 items-center justify-center rounded-lg">
        {secciones.map(({ id, nombre, icono }) => (
          <button
            key={id}
            className={`cursor-pointer flex flex-col justify-center items-center p-4 rounded-lg shadow-md w-32 h-32 sm:w-40 sm:h-40 md:w-52 md:h-52 transition-all ${vistaActual === id ? "bg-gray-400" : "bg-white hover:bg-gray-200"
              }`}
