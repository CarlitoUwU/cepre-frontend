...
export const Gestionar = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row h-[85vh] m-5 mt-25">
      ...
      <div className="w-full md:w-1/6 bg-gray-200 p-6 flex flex-wrap md:flex-col gap-4 md:gap-6 items-center justify-center md:justify-start rounded-lg max-h-screen overflow-auto">
        {secciones.map(({ id, nombre, icono }) => (
          <button
            key={id}
            className={`cursor-pointer flex flex-col justify-center items-center p-4 rounded-lg shadow-md w-28 h-28 sm:w-32 sm:h-32 md:w-40 md:h-40 transition-all ${
